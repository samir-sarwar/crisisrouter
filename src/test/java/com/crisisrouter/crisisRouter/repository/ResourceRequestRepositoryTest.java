...
class ResourceRequestRepositoryTest {
    ...
    private ResourceRequest createAndSaveRequest(String title, double lng, double lat, RequestStatus status) {
        return resourceRequestRepository.saveAndFlush(ResourceRequest.builder()
                ...
                .latitude(lat).longitude(lng).status(status).build());
    }
    ...
    void findNearbyRequests_withinRadius_returnsResults() {
        ...
        List<ResourceRequest> result = resourceRequestRepository.findNearbyRequests(43.4700, -80.5400, 5000.0);
    ...
    void findNearbyRequests_outsideRadius_returnsEmpty() {
        ...
        // Search from London, UK (~5500km away) with tiny radius
        List<ResourceRequest> result = resourceRequestRepository.findNearbyRequests(51.5074, -0.1276, 1.0);
    ...
    void findNearbyRequests_orderedByDistance() {
        ...
        List<ResourceRequest> result = resourceRequestRepository.findNearbyRequests(43.4695, -80.5390, 50000.0);
